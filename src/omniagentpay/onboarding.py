...
def create_env_file(
    ...
    env_content = f"""# OmniAgentPay Configuration
...
def quick_setup(
    ...
    - Recovery file in secure config directory (~/.config/omniagentpay/)
...
"""
    ...
    print()
    print("Ready to use:")
    print("  from omniagentpay import OmniAgentPay")
    print("  client = OmniAgentPay()")
...
def auto_setup_entity_secret(
    ...
    Called automatically by OmniAgentPay client when ENTITY_SECRET is missing.
    Saves recovery file to secure config directory (~/.config/omniagentpay/).
    ...
    if env_file.exists():
        with open(env_file, "a") as f:
            f.write(f"\n# Auto-generated by OmniAgentPay\nENTITY_SECRET={entity_secret}\n")
