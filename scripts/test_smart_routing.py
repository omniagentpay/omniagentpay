...
def test_smart_routing(router):
    ...
    method = router.detect_method(recipient, destination_chain="base")
    assert method == PaymentMethod.CROSSCHAIN
    ...
    assert transfer_adapter.supports(solana_recipient, source_network=Network.SOL) is True

    # And reject 0x address
    assert transfer_adapter.supports(recipient, source_network=Network.SOL) is False
...
async def test_execution_smart_routing(router, mock_deps):
    ...
    wallet_service.transfer = Mock(
        return_value=Mock(success=True, transaction=Mock(id="tx1", state=Mock(value="complete")))
    )
    ...
    result = await router.simulate(
        wallet_id="w1", recipient=recipient, amount=Decimal("10"), destination_chain="base"
    )

    assert result.route == PaymentMethod.CROSSCHAIN
    assert (
        not result.would_succeed
    )  # Expected false as balance check mocked incompletely or CCTP not ready

    # 2. Simulate Transfer via Default
    result = await router.simulate(wallet_id="w1", recipient=recipient, amount=Decimal("10"))
