...
async def test_payment_intents():
    ...
    client._router.simulate = AsyncMock(
        return_value=SimulationResult(
            would_succeed=True, route=PaymentMethod.TRANSFER, reason="Mock Success"
        )
    )
    ...
    intent = await client.create_payment_intent(
        wallet_id=wallet_id, recipient="0xRecipient", amount=Decimal("10.00"), purpose="Test Intent"
    )
    ...
    intent2 = await client.create_payment_intent(
        wallet_id=wallet_id, recipient="0xRecipient", amount=Decimal("20.00")
    )
