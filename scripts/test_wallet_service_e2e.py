...
def test_wallet_set_operations(service: WalletService):
    ...
    try:
        ...
        print_info("Name: (not returned by Circle API for developer wallets)")
    ...
    try:
        ...
        if all_wallets:
            ...
            for w in all_wallets[:5]:  # Show first 5
                print(
                    f"     • {w.id[:8]}... | {w.address[:10]}...{w.address[-6:]} | {w.blockchain}"
                )
...
def test_transfer_operations(service: WalletService, wallet):
    ...
    try:
        ...
        if usdc_amount == Decimal("0"):
            ...
            print("     1. Go to Circle Faucet for testnet USDC")
        ...
        if usdc_amount >= Decimal("0.01"):
            ...
            if result.success:
                print_success("Transfer initiated successfully!")
                print_info(
                    f"Transaction ID: {result.transaction.id if result.transaction else 'N/A'}"
                )
                print_info(
                    f"State: {result.transaction.state.value if result.transaction else 'N/A'}"
                )
                if result.tx_hash:
